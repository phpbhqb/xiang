...
function AddFeedback($add){
	...
	for($i=1;$i<$count-1;$i++)
	{
		...
		if(strstr($br['filef'],",".$mf.","))//附件
		{
			if(!$pr['feedbacktfile'])
			{
				printerror("NotOpenFBFile","",9);
			}
			if(!$_FILES[$mf]['name'])
			{
				printerror("EmptyFeedbackname","",9);
			}
		}
		else
		{
	...
	if($sql)
	{
		$reurl=$public_r[newsurl].'html/contact.html';
		printerror("AddFeedbackSuccess",$reurl,9);
	}
	else
